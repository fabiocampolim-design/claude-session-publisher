...
- **What compiles is checked, not just what exits 0** — the LaTeX path splits
  ...
  the typesetter.
- Standard library only, one file, 296 checks in the test suite, pyflakes and
...
296 checks, run against the synthetic sessions in `examples/` —
...
checker byte-identical to the publishing playbook — brings it to 47 commits.
