...
- **What compiles is checked, not just what exits 0** — the LaTeX path splits
  ...
  the typesetter.
- Standard library only, one file, 428 checks in the test suite, pyflakes and
...
428 checks, run against the synthetic sessions in `examples/` —
...
checker byte-identical to the publishing playbook — brings it to 51 commits (2.7.2, a regression the real-data survival run caught after an all-green suite, and 2.7.3, its independent review, are the last two).
