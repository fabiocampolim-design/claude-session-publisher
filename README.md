...
- Standard library only, one file, 247 checks in the test suite, CI on
  Linux/Windows/macOS.
...
247 checks, run against the synthetic sessions in `examples/` —
...
| **Software** | — | All of it |
| **Validation** | Broke every build against hundreds of thousands of records from a real archive; caught the stale-page, overflow and layout defects; set the bar (*"this needs high accuracy"*); commissioned the review and code-review passes | The 247-check test suite and CI; the review-driven survival runs |
