...
- Standard library only, one file, 220 checks in the test suite, CI on
  Linux/Windows.
...
220 checks, run against the synthetic sessions in `examples/` —
...
| **Software** | — | All of it |
| **Validation** | Broke every build against hundreds of thousands of records from a real archive; caught the stale-page, overflow and layout defects; set the bar (*"this needs high accuracy"*) | The 220-check test suite and CI |
